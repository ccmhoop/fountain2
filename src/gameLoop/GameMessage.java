...
public class GameMessage extends Gameloop {

    private static boolean fountain = false;

    public void gameMessage(int size) {
        roomMessage(entity.getY(), entity.getX(), size);
    }

    private void roomMessage(int y, int x, int size) {
        if (y == 0 && x == 0) {
            caveEntrance();
        }
        ...
        if (Generator.gameGrid[y][x].equals("m")) {
            monster.moveMaelstrom(y, x, size);
